...
await createApp({
  ...
  instructions: [
    ...
    'isPublicDomain and isHighlight accept true only, and both are partial indexes — each omits genuinely matching objects and returns a few that do not match the query, so verify per object with met_get_object. hasImages=true includes copyrighted works without usable image URLs.',
